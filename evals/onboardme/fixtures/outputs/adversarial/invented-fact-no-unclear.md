...
An `export-audit` CLI run calls `CsvExporter` to create the export id, then calls `AuditWriter` to append a `manual-export` row to the `audit-log` stream.
...
- **`audit-log` stream** - audit events for exports and partner callbacks; written through AuditWriter, and only the CLI writes audit rows.
