...
- **OrderController** - HTTP entry for order creation; validates the request shape and records pending order intent.
- **FulfillmentWorker** - consumes `OrderPlaced` and drives payment, inventory, and final status updates.
- **PaymentAdapter** - wraps the Stripe charge call.
- **InventoryStore** - owns inventory reserve operations.

## Where the truth lives
- **`orders` table** - order lifecycle and status; written by OrderController for create and FulfillmentWorker for finalization.
...
## The seams
- **Cut here:** the `OrderPlaced` message contract - post-order behavior slots in as another consumer.
- **Cut here:** the PaymentAdapter boundary - payment provider behavior stays behind one module.
- **Not here:** order status transitions span OrderController and FulfillmentWorker, so lifecycle changes touch both sides.
