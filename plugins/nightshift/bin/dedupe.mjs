...
function dedupeKeyEquals(a, b) {
  return a.surface === b.surface && a.symptom === b.symptom && a.root_cause === b.root_cause;
}
function isOpen(f) {
  return !f.resolved_at;
}
function matchOpenFinding(key, openFindings2) {
  return openFindings2.find((f) => isOpen(f) && dedupeKeyEquals(f.dedupe_key, key));
}
function activeSuppression(key, suppressions, today) {
...
function runDedupe(opts) {
  if (!existsSync3(opts.candidatesPath))
    throw new Error(`candidates not found: ${opts.candidatesPath}`);
  const candidates = readJson(opts.candidatesPath);
