...
// src/lib/select-run.ts
function readK(manifestPath, lane) {
  if (!existsSync2(manifestPath)) throw new Error(`manifest not found: ${manifestPath}`);
...
function runSelect(opts) {
  if (!existsSync2(opts.vectorsPath)) throw new Error(`registry not found: ${opts.vectorsPath}`);
...
`
    ...
    process.stderr.write(`select: ${err.message}
`);
    process.exit(2);
  }
}
