...
**Be honest about what this path is.** The `evidence` field you write is a **run-dir-relative pointer, not a durable one**: the run dir under `.nightshift/.run/<run_id>/` is disposable and gets cleaned up. The launcher's copy step (A7) relocates the *bytes* — after the run it content-addresses each confirmed finding's evidence file and copies it into the ops home — but it does not yet rewrite the *stored reference*: `finding.evidence` still holds the run-dir path you wrote, so once the run dir is gone that pointer resolves nowhere and the dashboard renders "evidence no longer on disk." Closing that gap is a decision A7 has no business taking alone and is deliberately left open for A8. What you write here only has to be correct for the duration of the run — no durability hand-off happens yet, so don't write as if one does.
