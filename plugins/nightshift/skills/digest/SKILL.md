...
2. **Overdue registry areas + motionless findings.** Entries with `status: overdue` (well
   past `interval_days`), and open findings whose `first_seen`/`last_seen` show no movement
   — the coverage that is silently rotting.
3. **False-positive rate + proposed new entries awaiting approval.** Read `fpr_7d`/`fpr_30d`
...
- **Read-only.** This skill summarizes; it never files issues, edits the registry, or runs
  reviews. Those are `/nightshift:qa`, `/nightshift:design`, and `/nightshift:garden`.
